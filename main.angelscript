...
void musix(){
	LoadMusic("soundfx\\background.mp3");
	PlaySample("soundfx\\background.mp3");
	}
...
void ETHCallback_logo(ETHEntity@ thisEntity){
	...
	if(input.KeyDown(K_ENTER) and GetTime() - openingWait > 2000){
		LoadScene("scenes\\realScene.esc","onSceneLoad","onSceneLoop");
		}
	}
...
void robotDrawSprite(ETHEntity@ robot){
	...
	string path = "entities\\sprites" + "\\robot_" + whichsprite + "_" + standOrWalking + "_" + (1 + (GetTime() / 85) % numframes) + ".png";
